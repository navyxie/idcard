...
function idcardinfo(idcard){
	...
	var year = info.birthday.toString().slice(0,4);
	var month = info.birthday.toString().slice(4,6);
	var date = info.birthday.toString().slice(6,8);
	var curDate = new Date();
	if((1+curDate.getMonth() < parseInt(month)) || (1+curDate.getMonth() === parseInt(month) &&  curDate.getDate() < parseInt(date))){
		info.age = curDate.getFullYear() - parseInt(year) - 1;
	}else{
		info.age = curDate.getFullYear() - parseInt(year);
	}
